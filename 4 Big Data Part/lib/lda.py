...
class LDAHelper:
    ...
    def similarity_topic_lists(self, wtopics1, wtopics2):
        """
        ...
        for wtopic1 in wtopics1:
            ...
            for wtopic2 in wtopics2:
                ...
                words1 = list((zip(*topic1))[0])
                words2 = list((zip(*topic2))[0])
                if similarity(words1, words2) > 3:
                    sim += 1
        return sim
    ...
    def create_corpus(self, documents, dictionary=None, dictionary_filters=False):
        dic = dictionary
        if dic is None:
            dic = gensim.corpora.Dictionary(documents)
        if dictionary_filters:
            dic.filter_extremes(no_below=5, no_above=0.8)
        return [dic.doc2bow(text) for text in documents]
    ...
    def lda_topic(self, documents, dictionary=None, dictionary_filters=False):
        ...
        """
        corpus = create_corpus(documents, dictionary, dictionary_filters)
